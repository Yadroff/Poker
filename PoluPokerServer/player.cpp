...
Player::Player(const QString &name, const qint64 &id, const quint64 &seat, QObject *parent)
...
Player::Player(const Player &another) :
        name_(another.name()), id_(another.id_), seat_(another.seat_), combination_(another.combination_) {
    coins_ = DEFAULT_COINS;
}
...
Combination Player::combination() const {
    return combination_;
}

void Player::addCard(const Card &card) {
    combination_.addCard(card);
}
...
std::ostream &operator<<(std::ostream &os, const Player &player) {
    os << "NAME: " << player.name_.toStdString() << " COINS: " << player.coins_ << std::endl;
